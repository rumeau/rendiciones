...
class GroupController extends AbstractActionController
{
    ...
    {
        $queryGroups = $this->objectManager->createQueryBuilder();
        $queryGroups->select('g', 'COUNT(u) as users')
        ...
        $queryGroups->orderBy($sortParams->by, $sortParams->sort);

        $groups = new ZendPaginator(new DoctrinePaginatorAdapter(new DoctrinePaginator($queryGroups, false)));
    ...
    {
        $gid = $this->params()->fromQuery('id', 0);
        $group = $this->objectManager->getRepository('Registry\Entity\UserGroup')
            ->findOneBy(array( 'id' => $gid, 'isDefault' => 0 ));
